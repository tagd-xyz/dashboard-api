...
{
    private function createTagd(Retailer $retailer, string $brand, float $amount): Tagd
    {
     ...
     * @return void
     */
    public function test_ret_rep_cur_brand_get_request()
    {
        $retailer = $this->aRetailer();

        $tagds = $this->createTagd($retailer, 'Adidas', 10.50);
        $tagds = $this->createTagd($retailer, 'Nike', 20.50);

        $url = static::URL_RET_REP_CURRENCY . '?filter={"brands":["Adidas"]}';
